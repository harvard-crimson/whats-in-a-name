 ...
 *
 * https://github.com/sahilchinoy/public-trust
 *
...
(function($,undefined){"use strict";var assurePlayerSound=function(player){player=$(player);if(isNaN(player.data('org-vol')))player.data('org-vol',player.jPlayer('option','volume'));return player;},consoleLog=function(msg,debug){if(!!console&&!!console.log&&debug)console.log(msg);},setVolume=function(player,v){player.jPlayer('volume',v);player.data('volume',v);},fadeToPlayer=function(player,dur,from,to,callback,debug){player=assurePlayerSound(player);if(to>from&&!isNaN(player.data('volume'))&&player.data('volume')>from)from=player.data('volume');if(from>to&&!isNaN(player.data('volume'))&&player.data('volume')<from)from=player.data('volume');var diff=to-from,limit=dur<1?-1:dur/100,int=limit<1?0:dur/limit,m=diff/limit,x=0,fade=function(){if(x<=limit){var v=from+m*x;player.data('is-fading',true);if(isNaN(v))consoleLog('[player] #'+player.attr('id')+' ***NaN',debug);else{setVolume(player,v);player.data('fadeout',setTimeout(fade,int));consoleLog('[player] #'+player.attr('id')+' volume set to '+v,debug);}x++;}else{setVolume(player,to);consoleLog('[player] #'+player.attr('id')+' volume set to '+to+' -- end',debug);player.data('is-fading',false);if($.isFunction(callback))callback.call(player);}};if(diff!==0&&!isNaN(diff)){clearTimeout(player.data('fadeout'));fade();}else{consoleLog('[player] #'+player.attr('id')+' fade out skipped',debug);if($.isFunction(callback))callback.call(player);}return player;},fadeOutPlayer=function(player,dur,_in,_out,callback,debug){player=assurePlayerSound(player);_in=_in!=null&&!isNaN(_in)?_in:parseFloat(player.data('org-vol'),10);_out=_out!=null&&!isNaN(_out)?_out:0;return fadeToPlayer(player,dur,_in,_out,callback,debug);},fadeInPlayer=function(player,dur,_in,_out,callback,debug){player=assurePlayerSound(player);_in=_in!=null&&!isNaN(_in)?_in:0;_out=_out!=null&&!isNaN(_out)?_out:parseFloat(player.data('org-vol'),10);return fadeToPlayer(player,dur,_in,_out,callback,debug);},playerIsFading=function(player){return !!player.data('is-fading');};if($.isFunction($.jPlayer)&&!$.isFunction($.jPlayer.fadeTo)){$.extend($.jPlayer,{fadeTo:fadeToPlayer,fadeOut:fadeOutPlayer,fadeIn:fadeInPlayer,isFading:playerIsFading});if(!$.fn.jPlayerFade)$.extend($.fn,{jPlayerFade:function(debug){var t=$(this);return{to:function(dur,from,to,callback){return $.jPlayer.fadeTo(t,dur,from,to,callback,debug);},'in':function(dur,from,to,callback){return $.jPlayer.fadeIn(t,dur,from,to,callback,debug);},out:function(dur,from,to,callback){return $.jPlayer.fadeOut(t,dur,from,to,callback,debug);},isFading:function(){return playerIsFading(t);}};}});}})(jQuery);
...
window.APP_CONFIG={"REPOSITORY_ALT_URL":null,"SERVER_PROJECT_PATH":"/home/ubuntu/apps/whats_in_a_name","DEPLOY_TO_SERVERS":false,"S3_DEPLOY_URL":null,"DEPLOY_SERVICES":false,"DEBUG":true,"COPY_PATH":"","GITHUB_USERNAME":"sahilchinoy","DEFAULT_MAX_AGE":20,"ASSETS_MAX_AGE":86400,"ASSETS_S3_BUCKET":{"region":"","bucket_name":""},"REPOSITORY_NAME":"public-trust","STAGING_S3_BUCKET":{"region":"us-west-1","bucket_name":"dailycal"},"PROJECT_SLUG":"whats-in-a-name","PRODUCTION_SERVERS":[],"SERVERS":[],"SERVER_SERVICES":[["app","/home/ubuntu/apps/whats_in_a_name/repository","ini"],["uwsgi","/etc/init","conf"],["nginx","/etc/nginx/locations-enabled","conf"]],"SERVER_BASE_URL":null,"REPOSITORY_URL":"git@github.com:sahilchinoy/public-trust.git","COPY_GOOGLE_DOC_URL":"","PRODUCTION_S3_BUCKET":{"region":"","bucket_name":""},"SERVER_VIRTUALENV_PATH":"/home/ubuntu/apps/whats_in_a_name/virtualenv","S3_BASE_URL":"","S3_BUCKET":null,"DEPLOY_CRONTAB":false,"ASSETS_SLUG":"public-trust","SERVER_PYTHON":"python2.7","GOOGLE_ANALYTICS":{"DOMAIN":"","ACCOUNT_ID":""},"STAGING_SERVERS":[],"SERVER_USER":"ubuntu","PROJECT_FILENAME":"whats_in_a_name","SERVER_REPOSITORY_PATH":"/home/ubuntu/apps/whats_in_a_name/repository","SHARE_URL":"http:///whats-in-a-name/","UWSGI_SOCKET_PATH":"/tmp/whats_in_a_name.uwsgi.sock","SERVER_LOG_PATH":null,"DEPLOYMENT_TARGET":null};
var AUDIO=(function(){var narrativePlayer=null;var ambientPlayer=null;var ambientId=null;var progressInterval=null;var narrativeURL=null;var ambientURL=null;var narrativeVisible=false;var narrativeName=null;var checkForAudio=function(slideIndex){var $slide=$slides.eq(slideIndex);var narrativeFilename=$slide.attr('data-narrative');narrativeName=$slide.attr('data-narrative');if(narrativeFilename){$thisPlayerProgress=$slide.find('.player-progress');$playedBar=$slide.find('.player-progress .played');$controlBtn=$slide.find('.control-btn');narrativeURL=APP_CONFIG.S3_BASE_URL+'/assets/audio/'+narrativeFilename+'.m4a';setNarrativeMedia();}else if(slideIndex===$slides.length-2)_setUpGrid($slide);else{_pauseNarrativePlayer();narrativeVisible=false;}};var setUpNarrativePlayer=function(){$narrativePlayer.jPlayer({swfPath:'js/lib',loop:false,supplied:'m4a',timeupdate:onNarrativeTimeupdate,loadeddata:subtitles});};var _playGridAudio=function($box){$('.img-box').removeClass('playing');$box.addClass('playing');var narrativeFilename=$box.data('audio');narrativeURL=APP_CONFIG.S3_BASE_URL+'/assets/audio/'+narrativeFilename+'.m4a';setNarrativeMedia();};var _setUpGrid=function($slide){$thisPlayerProgress=$slide.find('.player-progress');$playedBar=$slide.find('.player-progress .played');$controlBtn=$slide.find('.control-btn');$('.img-box').click(function(e){_playGridAudio($(this));});_playGridAudio($('.img-box.first'));};var setNarrativeMedia=function(){_startNarrativePlayer();};var _startNarrativePlayer=function(){$narrativePlayer.jPlayer('setMedia',{m4a:narrativeURL});narrativeVisible=true;setTimeout(function(){if(narrativeVisible){$narrativePlayer.jPlayer('play');$controlBtn.removeClass('play').addClass('pause');}},1000);};var _resumeNarrativePlayer=function(){$narrativePlayer.jPlayer('play');$controlBtn.removeClass('play').addClass('pause');var remTime=$narrativePlayer.data().jPlayer.status.duration-$narrativePlayer.data().jPlayer.status.currentTime;$('#'.concat(narrativeName)).animate({top:"-80%"},remTime*1000,subtitles);};var _pauseNarrativePlayer=function(end){$narrativePlayer.jPlayer('pause');if(end)$playedBar.css('width',$thisPlayerProgress.width()+'px');$controlBtn.removeClass('pause').addClass('play');$('.subtitle-text').stop();};var toggleNarrativeAudio=function(){if($narrativePlayer.data().jPlayer.status.paused)_resumeNarrativePlayer();else _pauseNarrativePlayer(false);};var fakeNarrativePlayer=function(){$narrativePlayer.jPlayer('setMedia',{m4a:APP_CONFIG.S3_BASE_URL+'/assets/audio/'+'reich_careful.m4a'}).jPlayer('pause');};var onNarrativeTimeupdate=function(e){if(narrativeVisible){var totalTime=e.jPlayer.status.duration;var position=e.jPlayer.status.currentTime;var percentage=position/totalTime;if(position===0){$playedBar.addClass('no-transition');$playedBar.css('width',0);}else{$playedBar.removeClass('no-transition');$playedBar.css('width',$thisPlayerProgress.width()*percentage+'px');if(percentage===1)$controlBtn.removeClass('pause').addClass('play');}}};var animateSubtitles=function(position){if(subtitles){var activeSubtitle=null;for(var i=0;i<subtitles.length;i++)if(position>0)if(position<subtitles[i].time){if(i>0)activeSubtitle=subtitles[i-1].transcript;else activeSubtitle=subtitles[i].transcript;$subtitleWrapper.fadeIn();$subtitles.html(activeSubtitle);break;}else{activeSubtitle=subtitles[i].transcript;$subtitles.html(activeSubtitle);}}};var setUpAmbientPlayer=function(){$ambientPlayer.jPlayer({swfPath:'js/lib',supplied:'m4a',loop:true,volume:0.1});};var setAmbientMedia=function(url){$ambientPlayer.jPlayer('setMedia',{m4a:url}).jPlayer('play');};var fakeAmbientPlayer=function(){$ambientPlayer.jPlayer('setMedia',{m4a:APP_CONFIG.S3_BASE_URL+'/assets/audio/'+'doctor_ambient.m4a'}).jPlayer('pause');};var toggleAllAudio=function(){if(isHidden()){if(narrativeVisible)_pauseNarrativePlayer(false);$ambientPlayer.jPlayer('pause');}else{if(narrativeVisible)_resumeNarrativePlayer();$ambientPlayer.jPlayer('play');}};var subtitles=function(e){var totalTime=e.jPlayer.status.duration;$('#'.concat(narrativeName)).css('top','');$('#'.concat(narrativeName)).animate({top:"-80%"},(totalTime+10)*1000,subtitles);};return{'checkForAudio':checkForAudio,'toggleNarrativeAudio':toggleNarrativeAudio,'toggleAllAudio':toggleAllAudio,'setUpAmbientPlayer':setUpAmbientPlayer,'setUpNarrativePlayer':setUpNarrativePlayer,'fakeNarrativePlayer':fakeNarrativePlayer};}());
var $upNext=null;var $w;var $h;var $slides;var $arrows;var $nextArrow;var $controlBtn;var $thisPlayerProgress;var $playedBar;var $subtitleWrapper;var $subtitles;var $slideTitle;var $ambientPlayer;var $narrativePlayer;var $progressIndicator;var $currentProgress;var isTouch=Modernizr.touch;var mobileSuffix;var aspectWidth=16;var aspectHeight=9;var optimalWidth;var optimalHeight;var w;var h;var completion=0;var visibilityProperty=null;var resize=function(){$w=$(window).width();$h=$(window).height();$slides.width($w);optimalWidth=($h*aspectWidth)/aspectHeight;optimalHeight=($w*aspectHeight)/aspectWidth;w=$w;h=optimalHeight;if(optimalWidth>$w){w=optimalWidth;h=$h;}};var setUpFullPage=function(){$.fn.fullpage({anchors:false,autoScrolling:false,keyboardScrolling:false,verticalCentered:false,fixedElements:'.primary-navigation, .progress-indicator',resize:false,css3:true,loopHorizontal:false,afterRender:onPageLoad,afterSlideLoad:lazyLoad});};var onPageLoad=function(){setSlidesForLazyLoading(0);$('.section').css({'opacity':1,'visibility':'visible'});showNavigation();};var lazyLoad=function(anchorLink,index,slideAnchor,slideIndex){setSlidesForLazyLoading(slideIndex);showNavigation();AUDIO.checkForAudio(slideIndex);animateProgress(slideIndex);};var setSlidesForLazyLoading=function(slideIndex){var slides=[$slides.eq(slideIndex),$slides.eq(slideIndex+1),$slides.eq(slideIndex+2),$slides.eq(slideIndex+3),$slides.eq(slideIndex+4)];mobileSuffix='';for(var i=0;i<slides.length;i++)loadImages(slides[i]);;};var loadImages=function($slide){if($slide.data('bgimage')){var image_filename=$slide.data('bgimage').split('.')[0];var image_extension='.'+$slide.data('bgimage').split('.')[1];var image_path='assets/'+image_filename+image_extension;if($slide.css('background-image')==='none')$slide.css('background-image','url('+image_path+')');}var $images=$slide.find('img.lazy-load');if($images.length>0)for(var i=0;i<$images.length;i++){var image=$images.eq(i).data('src');var image_path='assets/'+image+mobileSuffix+'.jpg';$images.eq(i).attr('src',image_path);}};var showNavigation=function(){if($slides.first().hasClass('active')){if(!$arrows.hasClass('active'))showArrows();$prevArrow.removeClass('active');$prevArrow.hide();$nextArrow.css('animation','blinker 4s linear infinite');$nextArrow.on('click',onFirstRightArrowClick);}else if($slides.last().hasClass('active')){if(!$arrows.hasClass('active'))showArrows();$nextArrow.removeClass('active');$nextArrow.hide();}else{$nextArrow.css('animation','none');if($arrows.filter('active').length!=$arrows.length)showArrows();}$progressIndicator.show();};var showArrows=function(){$arrows.addClass('active');$arrows.show();};var animateProgress=function(index){var totalSlides=$slides.length;var percentage=(index+1)/totalSlides;$currentProgress.css('width',percentage*100+'%');if(index===0)$progressIndicator.width(0);else $progressIndicator.width('100%');};var onFirstRightArrowClick=function(){if(isTouch)if($slides.first().hasClass('active'))AUDIO.fakeNarrativePlayer();};var onDocumentKeyDown=function(e){if(e.which===37||e.which===39)if(e.which===37)$.fn.fullpage.moveSlideLeft();else if(e.which===39)$.fn.fullpage.moveSlideRight();return true;};var fakeMobileHover=function(){$(this).css({'opacity':1});};var rmFakeMobileHover=function(){$(this).css({'opacity':.3});};var onControlBtnClick=function(e){e.preventDefault();AUDIO.toggleNarrativeAudio();e.stopPropagation();reloadSubtitle();};var reloadSubtitle=function(){$('.subtitles').stop();};var onVisibilityChange=function(){AUDIO.toggleAllAudio();};var getHiddenProperty=function(){var prefixes=['webkit','moz','ms','o'];if('hidden' in document)return 'hidden';for(var i=0;i<prefixes.length;i++)if((prefixes[i]+'Hidden') in document)return prefixes[i]+'Hidden';return null;};var isHidden=function(){var prop=getHiddenProperty();if(!prop)return false;return document[prop];};var setUpGraphics=function(){$('.graphic-embed').each(function(){var graphic=$(this).data('graphic');var pymParent=new pym.Parent($(this).attr('id'),'https://s3-us-west-1.amazonaws.com/dc-graphics/dailygraphics/graphics/'+graphic+'/child.html',{});});};$(document).ready(function(){$w=$(window).width();$h=$(window).height();$slides=$('.slide');$navButton=$('.primary-navigation-btn');$arrows=$('.controlArrow');$prevArrow=$arrows.filter('.prev');$nextArrow=$arrows.filter('.next');$upNext=$('.up-next');$controlBtn=$('.control-btn');$narrativePlayer=$('#narrative-player');$ambientPlayer=$('#ambient-player');$progressIndicator=$('.progress-indicator');$currentProgress=$('.current-progress');$navbaritems=$('.nav-item');$nav1=$('#nav1');$nav2=$('#nav2');$nav3=$('#nav3');$nav4=$('#nav4');$controlBtn.on('click',onControlBtnClick);$arrows.on('touchstart',fakeMobileHover);$arrows.on('touchend',rmFakeMobileHover);$(document).keydown(onDocumentKeyDown);AUDIO.setUpNarrativePlayer();setUpFullPage();setUpGraphics();resize();window.addEventListener("deviceorientation",resize,true);$(window).resize(resize);visibilityProperty=getHiddenProperty();if(visibilityProperty){var evtname=visibilityProperty.replace(/[H|h]idden/,'')+'visibilitychange';document.addEventListener(evtname,onVisibilityChange);}});
function fun(){$('.subtitles').css('top','');$('.subtitles').animate({top:"-100%"},15000,fun);}fun();